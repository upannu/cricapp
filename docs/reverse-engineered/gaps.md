...
| MKT-GAP-10 | "Credit to Pack" no-op (MKT-015) | **Fixed** — the whole "Credit to Pack" button (and the rest of the vestigial pack-on-booking plumbing) was removed. Session Packs now only ever fund the weekly group net; a booking is always its own fee. See PR "Keep Session Packs to the weekly group". | Resolved by removal — no longer a code path. | — |
...
| PORTAL-GAP-006 | Bulk messaging | **FIXED.** `BulkMessageModal` now calls `/api/send-message`/`/api/send-sms` per recipient (mirroring `MessageModal`'s single-recipient flow) before logging that recipient's message row; a failed delivery is excluded from the log and reflected in the UI's own success/failure count instead of being silently claimed as sent. | Coaches using "Bulk Message" can now trust the delivery claim. | Regression-guarded by `tests/e2e/roles/coach/bulk-message-delivery.spec.ts` (flipped from documenting the defect to asserting the fix) and `tests/components/BulkMessageModal.test.tsx`. |
...
| Bulk messaging actual delivery | IMPLEMENTED — fixed (PORTAL-GAP-006) |
...
| 5 | `MKT-GAP-10` | The "Credit to Pack" button on the Bookings page is still a confirmed silent no-op — `sessionCredits` never actually increments despite a success message. | **Fixed** — button and the vestigial pack-on-booking plumbing removed; packs now only fund the weekly group net. See the MKT-GAP-10 row above. |
| 6 | `PORTAL-GAP-006` | `BulkMessageModal` now calls `/api/send-message`/`/api/send-sms` per recipient before logging, instead of logging unconditionally. | **Fixed** — see PORTAL-GAP-006 entry above. |
