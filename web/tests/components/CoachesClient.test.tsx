...
const { fetchCoaches, fetchAcademies, fetchPlayers, upsertCoach, updateCoachFields, reassignCoachPlayers } = vi.hoisted(() => ({
...
vi.mock("@/lib/db", () => ({
  fetchCoaches, fetchAcademies, fetchPlayers, upsertCoach, updateCoachFields, reassignCoachPlayers,
...
describe("CoachesClient", () => {
  ...
  test("Edit navigates to the coach's own dedicated edit page, not an inline form", async () => {
    const user = userEvent.setup();
    setupDefaults();
    fetchCoaches.mockResolvedValue([makeCoach({ id: "c1", name: "Coach Dan" })]);
    ...
    expect(push).toHaveBeenCalledWith("/coaches/c1/edit");
