...
export async function middleware(request: NextRequest) {
  ...
  const isAlwaysPublicPage =
    ...
    // Cricket Board Partnership landing/apply/success pages, the Organisations hub, specialist
    // program pages (e.g. /programs/fast-bowling-development), and the Platform/Performance
    // marketing pages — any prospect needs to reach these signed out, and a signed-in staff member
    // should be able to open/share them too (e.g. to walk a prospect through the flow).
    pathname.startsWith("/partnerships") ||
    pathname.startsWith("/organisations") ||
    pathname.startsWith("/programs") ||
    pathname.startsWith("/platform") ||
    pathname.startsWith("/performance-intelligence");
