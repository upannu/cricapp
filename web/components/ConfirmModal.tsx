...
const CONFIRM_BUTTON_CLASSES: Record<NonNullable<ConfirmModalProps["confirmVariant"]>, string> = {
  default: "bg-hp-cg text-hp-paper hover:bg-hp-cg/90",
...
export function ConfirmModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-hp-surface w-full max-w-sm shadow-2xl border border-white/15 p-6">
        <div className={`w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-4 ${iconBg}`}>
          {icon}
        </div>
        <h3 className="font-display font-black uppercase text-hp-paper text-center tracking-wide mb-1">{title}</h3>
        <p className={`text-hp-paper/60 text-sm text-center ${children ? "mb-4" : "mb-6"}`}>{message}</p>
        {children && <div className="mb-6">{children}</div>}
        {error && <p className="text-red-400 text-xs text-center mb-4">{error}</p>}
        <div className="flex gap-3">
          <button type="button" onClick={onCancel} disabled={loading}
            className="flex-1 px-4 py-2.5 text-sm font-medium text-hp-paper/60 border border-white/15 hover:text-hp-paper hover:border-white/30 transition-colors cursor-pointer disabled:opacity-60">
            {cancelLabel}
          </button>
          <button type="button" onClick={onConfirm} disabled={loading}
            className={`flex-1 px-4 py-2.5 text-sm font-bold transition-colors cursor-pointer disabled:opacity-60 ${CONFIRM_BUTTON_CLASSES[confirmVariant]}`}>
