...
export function MembershipProfileClient({ packId }: { packId: string }) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-8">
      <Link href="/session-packs" className="text-xs text-zinc-400 hover:text-white transition-colors mb-4 inline-block">
        ← Back to Memberships
      </Link>

      <div className="bg-surface rounded-2xl p-6 border border-transparent">
        {/* Player header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap mb-0.5">
                ...
                <span className="text-zinc-400 text-xs">{player.ageGroup} · {getCoachOrAcademyLabel(player, coaches, academies)}</span>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="grid grid-cols-4 gap-3 mb-4">
              ...
              <PackStat label="Credits" value={String(pack.sessionCredits)} sub="returned" color={pack.sessionCredits > 0 ? "text-blue-400" : "text-zinc-600"} />
            ...
            {pack.feePerSession === 0 && academyWaivesFees(pack.academyId) ? (
              ...
              <div className="bg-ink rounded-xl p-4 grid grid-cols-3 gap-3 text-center mb-4">
                <div>
                  <div className="text-sm font-bold text-white">{formatMoney(pack.feePerSession, academy?.currency ?? DEFAULT_CURRENCY)}/session</div>
                ...
                <div>
                  ...
                  <div className="text-xs text-zinc-500 mt-0.5">Platform ({getPlatformFeePercent(pack.academyId, academies, plans)}%)</div>
                </div>
                <div>
                  ...
                  {relevantGroupSessions.map((g) => {
                    ...
                    return (
                      <label key={g.id} className={`flex items-center gap-2.5 rounded-lg px-2.5 py-1.5 cursor-pointer transition-colors ${
                        ...
                        <input type="checkbox" checked={checked} onChange={() => handleToggleGroupSession(g)} className="accent-pace-green" />
          ...
          {packActivity.length === 0 ? (
            ...
            <div className="space-y-1.5">
              ...
              {packActivity.length > 5 && (
                <p className="text-xs text-zinc-500 text-center pt-1">+{packActivity.length - 5} more</p>
              )}
            </div>
          )}
        </div>
