...
function subStatusStyle(status: string | undefined): string {
  ...
  if (status === "canceled") return "bg-white/10 text-hp-paper/45";
  return "bg-white/10 text-hp-paper/45";
}
...
export function PlatformKpisClient() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-8">
      <div className="mb-8">
        <h1 className="font-display font-black uppercase text-2xl text-hp-paper tracking-wide mb-1">Platform KPIs</h1>
        <p className="text-hp-paper/45 text-sm">Cross-academy overview — plans, subscription status, coaches</p>
      </div>
        ...
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-4 mb-6">
            <div className="bg-hp-surface p-5 text-center">
              <div className="text-2xl font-bold text-hp-paper mb-1">{academies.length}</div>
              <div className="text-xs text-hp-paper/45">Total academies</div>
            </div>
            <div className="bg-hp-surface p-5 text-center">
              <div className="text-2xl font-bold text-pace-green mb-1">{activeCount}</div>
              <div className="text-xs text-hp-paper/45">Active academies</div>
            </div>
            <div className="bg-hp-surface p-5 text-center">
              <div className="text-2xl font-bold text-blue-400 mb-1">{coaches.length}</div>
              <div className="text-xs text-hp-paper/45">Total coaches</div>
            </div>
            <div className="bg-hp-surface p-5 text-center">
              <div className="text-2xl font-bold text-amber mb-1">{totalPlayers}</div>
              <div className="text-xs text-hp-paper/45">Total players</div>
            </div>
            <div className="bg-hp-surface p-5 text-center">
              <div className={`text-2xl font-bold mb-1 ${attentionCount > 0 ? "text-fire" : "text-hp-paper"}`}>{attentionCount}</div>
              <div className="text-xs text-hp-paper/45">Needs attention</div>
            </div>
          </div>

          {/* Plan distribution */}
          <div className="bg-hp-surface p-5 mb-6">
            <h2 className="text-xs font-mono font-semibold uppercase tracking-widest text-hp-paper/45 mb-4">Plan distribution</h2>
            <div className="space-y-2">
              {planCounts.map(({ plan, count }) => (
                <div key={plan.id} className="flex items-center justify-between text-sm py-1.5 border-b border-white/8 last:border-0">
                  <div className="flex items-center gap-2 min-w-0">
                    <span className="text-hp-paper font-medium truncate">{plan.name}</span>
                    <span className="text-hp-paper/45 text-xs flex-shrink-0">
                      {plan.billingType === "one_time" ? "one-time" : `/${plan.billingInterval}`} · {formatMoney(plan.priceAud, "aud")}
                    </span>
                  </div>
                  <span className="text-hp-paper/70 font-bold flex-shrink-0">{count}</span>
                </div>
              ))}
              {noPlanCount > 0 && (
                <div className="flex items-center justify-between text-sm py-1.5">
                  <span className="text-hp-paper/45">No plan assigned</span>
                  <span className="text-hp-paper/45 font-bold">{noPlanCount}</span>
                </div>
              )}
              {planCounts.length === 0 && noPlanCount === 0 && (
                <div className="text-hp-paper/45 text-sm">No academies yet.</div>
              )}
            </div>
          </div>
          ...
          <div className="bg-hp-surface overflow-hidden">
            <h2 className="text-xs font-mono font-semibold uppercase tracking-widest text-hp-paper/45 px-5 pt-5 mb-3">Academies</h2>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-left text-hp-paper/45 text-xs border-b border-white/8">
                ...
                <tbody>
                  {sortedAcademies.map((a) => (
                    <tr key={a.id} className="border-b border-white/8 last:border-0">
                      <td className="px-5 py-2.5 text-hp-paper font-medium">{a.name}</td>
                      <td className="px-3 py-2.5">
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                          a.status === "Active" ? "bg-pace-green/20 text-pace-green" : "bg-white/10 text-hp-paper/45"
                        }`}>{a.status}</span>
                      </td>
                      <td className="px-3 py-2.5">
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${STAGE_STYLES[a.stage]}`}>{a.stage}</span>
                      </td>
                      <td className="px-3 py-2.5 text-hp-paper/70">{planName(a.planId)}</td>
                      <td className="px-3 py-2.5">
                      ...
                      <td className="px-3 py-2.5 text-center text-hp-paper/70">{coachCount(a)}</td>
                      <td className="px-3 py-2.5 text-center text-hp-paper/70">{a.playerIds.length}</td>
                    </tr>
                  ))}
                  {sortedAcademies.length === 0 && (
                    <tr><td colSpan={7} className="px-5 py-6 text-center text-hp-paper/45">No academies yet.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
