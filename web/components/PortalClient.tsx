...
const PRIORITY_STYLES: Record<string, string> = {
  ...
  Low: "bg-white/10 text-hp-paper/45 border-white/15",
};
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-8 space-y-6">
      ...
      <div className="flex items-center gap-4">
        <div className="w-16 h-16 rounded-full bg-hp-cg flex items-center justify-center text-hp-paper font-bold text-2xl flex-shrink-0">
          {initials}
        </div>
        <div>
          <h1 className="font-display font-black uppercase text-2xl text-hp-paper tracking-wide">{player.name}</h1>
          <p className="text-hp-paper/45 text-sm">{player.bowlingStyle} · {player.ageGroup} · {player.club}</p>
        </div>
        <div className="ml-auto text-right">
          <div className="text-pace-green font-bold text-lg">⚡ {player.xp.toLocaleString()} XP</div>
          <Link href={`/players/${player.id}/subscription`} className="text-xs text-hp-paper/45 hover:text-pace-green hover:underline transition-colors">
      ...
      {player.subscription.plan === "Free" ? (
        <Link href={`/players/${player.id}/subscription`}
          className="block bg-pace-green/10 p-5 border border-pace-green/40 hover:border-pace-green/70 transition-colors">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-hp-paper font-semibold text-sm">Upgrade to Player Pro</p>
              <p className="text-hp-paper/45 text-xs mt-0.5">Unlimited sessions logged, AI biomechanics reports, and full Find a Coach access.</p>
            </div>
        ...
        <Link href={`/players/${player.id}/subscription`}
          className="block bg-hp-surface p-5 border border-white/12 hover:border-white/25 transition-colors">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-hp-paper font-semibold text-sm">Manage Subscription</p>
              <p className="text-hp-paper/45 text-xs mt-0.5">You&apos;re on {player.subscription.plan}. Update payment, switch plans, or cancel any time.</p>
            </div>
            <span className="text-hp-paper/45 text-sm font-bold flex-shrink-0">Manage →</span>
          </div>
        </Link>
      )}
      ...
      <Link href="/portal/log-session"
        className="block bg-hp-surface p-5 border border-hp-cg/30 hover:border-hp-cg/60 transition-colors">
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="text-hp-paper font-semibold text-sm">Log My Own Session</p>
            <p className="text-hp-paper/45 text-xs mt-0.5">No coach around? Upload your own delivery video for AI analysis.</p>
          </div>
          <span className="text-hp-cg text-sm font-bold flex-shrink-0">+ Log Session →</span>
        </div>
      </Link>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="bg-hp-surface p-5">
          <p className="text-xs font-semibold uppercase tracking-wider text-hp-paper/45 mb-3">Your Coach</p>
          {coach ? (
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                <div className="text-hp-paper font-semibold text-sm truncate">{coach.name}</div>
                <div className="text-hp-paper/45 text-xs truncate">{coach.specialization || coach.certificationLevel}</div>
                {coach.email && <a href={`mailto:${coach.email}`} className="text-pace-green text-xs hover:underline">{coach.email}</a>}
                {coach.phone && <div className="text-hp-paper/45 text-xs">{coach.phone}</div>}
              </div>
            </div>
          ) : (
            <p className="text-hp-paper/45 text-sm">No coach assigned yet.</p>
          )}
        </div>
        <div className="bg-hp-surface p-5">
          <p className="text-xs font-semibold uppercase tracking-wider text-hp-paper/45 mb-3">Next Booking</p>
          {(() => {
            ...
            if (!upcoming) return <p className="text-hp-paper/45 text-sm">No upcoming sessions scheduled.</p>;
            return (
              <div className="text-sm">
                <div className="text-hp-paper font-semibold">{upcoming.type}</div>
                <div className="text-hp-paper/45 text-xs mt-0.5">{formatDate(upcoming.date)} · {upcoming.time}</div>
                {upcoming.location && <div className="text-hp-paper/45 text-xs mt-0.5">{upcoming.location}</div>}
              </div>
            );
          })()}
        </div>
      ...
      {(activePack || squadGroups.length > 0) && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {activePack && (
            <div className="bg-hp-surface p-5">
              <p className="text-xs font-semibold uppercase tracking-wider text-hp-paper/45 mb-3">My Membership</p>
          ...
          {squadGroups.length > 0 && (
            <div className="bg-hp-surface p-5">
              <p className="text-xs font-semibold uppercase tracking-wider text-hp-paper/45 mb-3">Upcoming Squad Training</p>
              {upcomingSquadTraining.length === 0 ? (
                <p className="text-hp-paper/45 text-sm">No squad training scheduled in the next two weeks.</p>
              ) : (
                <div className="space-y-2">
                  {upcomingSquadTraining.map(({ group, date }) => (
                    <div key={`${group.id}-${date}`} className="text-sm">
                      <div className="text-hp-paper font-semibold">{group.name}</div>
                      <div className="text-hp-paper/45 text-xs mt-0.5">
      ...
      {tip && (
        <div className="bg-hp-surface p-5">
          <div className="flex items-center justify-between mb-2">
            <p className="text-xs font-semibold uppercase tracking-wider text-hp-paper/45">Today&apos;s Academy Tip</p>
      ...
      {(() => {
        ...
        return (
          <div className={`p-5 border ${
            ...
            <p className="text-xs font-semibold uppercase tracking-wider text-hp-paper/45 mb-2">{cardLabel}</p>
            {player.guardianConsentStatus === "Confirmed" ? (
              <div>
              ...
              <div>
                <p className="text-hp-paper/70 text-sm mb-3">
                  {isMinor
                ...
                <button
                  ...
                  disabled={confirming}
                  className="px-4 py-2.5 text-sm font-bold bg-pace-green text-black hover:opacity-90 transition-opacity cursor-pointer disabled:opacity-60"
                >
              ...
              <p className="text-hp-paper/70 text-sm">Awaiting confirmation from {player.name}&apos;s guardian.</p>
            )}
          </div>
        );
      })()}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="bg-hp-surface p-5">
          <p className="text-xs font-semibold uppercase tracking-wider text-hp-paper/45 mb-3">Latest Biomechanics</p>
          <div className="space-y-2 text-sm">
        ...
        <div className="bg-hp-surface p-5">
          <p className="text-xs font-semibold uppercase tracking-wider text-hp-paper/45 mb-3">Academy Progress</p>
          <div className="space-y-2 text-sm">
      ...
      {actionPlans.length > 0 && (
        <div>
          <p className="text-xs font-bold uppercase tracking-wider text-hp-paper/45 mb-3">Action Plans</p>
          <div className="space-y-3">
            {actionPlans.map((ap) => (
              <div key={ap.id} className="bg-hp-surface p-4">
                <div className="flex flex-wrap items-center gap-2 mb-1.5">
                  <span className="text-hp-paper font-semibold text-sm">{ap.title}</span>
                  <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold border ${PRIORITY_STYLES[ap.priority] ?? ""}`}>
                    {ap.priority}
                  </span>
                  <span className="text-hp-paper/45 text-xs">{ap.status}</span>
                  {ap.dueDate && <span className="text-hp-paper/45 text-xs">· Due {formatDate(ap.dueDate)}</span>}
                </div>
                {ap.drills.length > 0 && (
                  <ul className="text-hp-paper/70 text-sm list-disc list-inside space-y-0.5 mb-1.5">
                    {ap.drills.map((d, i) => <li key={i}>{d}</li>)}
                  </ul>
                )}
                {ap.notes && <p className="text-hp-paper/45 text-xs">{ap.notes}</p>}
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Recent sessions */}
      <div>
        <p className="text-xs font-bold uppercase tracking-wider text-hp-paper/45 mb-3">Recent Sessions</p>
        {sessions.length === 0 ? (
          <div className="bg-hp-surface p-8 text-center text-hp-paper/45 text-sm">No sessions logged yet.</div>
        ) : (
          <div className="space-y-2">
            {sessions.slice(0, 10).map((s) => (
              <div key={s.id} className="bg-hp-surface px-4 py-3 flex items-center justify-between gap-3">
                <div>
                  <div className="text-hp-paper text-sm font-semibold">{s.type}</div>
                  <div className="text-hp-paper/45 text-xs">{formatDate(s.date)} · {s.videos.length} video{s.videos.length !== 1 ? "s" : ""}</div>
                </div>
                <div className="text-xs text-hp-paper/45">+{s.xpEarned} XP</div>
              </div>
            ))}
          </div>
        )}
      ...
      {/* Recent reports */}
      <div>
        <p className="text-xs font-bold uppercase tracking-wider text-hp-paper/45 mb-3">Reports</p>
        {reports.length === 0 ? (
          aiReportsIncludedForPlayer(player, plans, academies, coach ? [coach] : []) ? (
            <div className="bg-hp-surface p-8 text-center text-hp-paper/45 text-sm">
              No reports yet — one will appear here after your next session with video.
            </div>
          ) : (
            <div className="bg-hp-surface p-8 text-center">
              <p className="text-hp-paper/45 text-sm mb-3">🔒 AI biomechanics reports require Player Pro or higher.</p>
              <Link
                href={`/players/${player.id}/subscription`}
                className="inline-block px-4 py-2 text-xs font-bold bg-pace-green text-black hover:opacity-90 transition-opacity"
              >
                Upgrade to unlock
              </Link>
            </div>
          ...
          <div className="space-y-3">
            {reports.slice(0, 10).map((r) => (
              <div key={r.id} className="bg-hp-surface p-4">
                <div className="flex flex-wrap items-center gap-2 mb-1.5">
                  <span className="px-2 py-0.5 rounded-full text-xs font-semibold bg-pace-green/10 text-pace-green">{r.type}</span>
                  <span className="text-hp-paper/45 text-xs">{formatDate(r.date)}</span>
                  {r.overallScore !== null && r.overallScore !== undefined && (
                    <span className="text-xs font-mono text-hp-paper">{r.overallScore}/100</span>
                  )}
                  {r.injuryRisk && (
                ...
                <p className="text-hp-paper/70 text-sm leading-relaxed">{r.summary}</p>
                {r.sessionId && (
                  <a
...
function UnpaidPackCard({ pack, currency }: { pack: SessionPack; currency: Currency }) {
  ...
  return (
    <div className={`p-4 flex items-center justify-between gap-4 flex-wrap border ${
      ...
      <button type="button" onClick={handlePay} disabled={loading}
        className={`px-4 py-2 text-xs font-bold transition-colors flex-shrink-0 disabled:opacity-60 cursor-pointer border ${
...
function Row({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div className="flex items-center justify-between">
      <span className="text-hp-paper/45">{label}</span>
      <span className="text-hp-paper font-semibold">{value}</span>
    </div>
  );
}
