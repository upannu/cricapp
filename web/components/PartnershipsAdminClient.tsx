...
export function PartnershipsAdminClient() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-8">
      <h1 className="font-display font-black uppercase text-2xl text-hp-paper tracking-wide mb-1">Cricket Board Partnerships</h1>
      <p className="text-sm text-hp-paper/45 mb-6">
      ...
      <div className="mb-4">
        <input type="text" value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }}
          ...
          className="w-full max-w-sm bg-hp-surface px-4 py-2.5 text-hp-paper placeholder-hp-paper/30 border border-white/12 focus:border-hp-cg focus:outline-none text-sm" />
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-hp-surface p-10 text-center">
          <p className="text-hp-paper/45 text-sm">{applications.length === 0 ? "No applications yet." : "No applications match your search."}</p>
        </div>
      ) : (
        <>
          <div className="bg-hp-surface overflow-hidden overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {pageRows.map((a) => (
                  <tr key={a.id} className="border-b border-white/8 last:border-0 hover:bg-white/[0.02]">
                    <td className="px-4 py-4">
                      <p className="text-hp-paper font-semibold">{a.organisationName}</p>
                      <p className="text-hp-paper/45 text-xs">{a.reference}</p>
                    </td>
                    <td className="px-4 py-4 text-hp-paper/70">{a.organisationType}</td>
                    <td className="px-4 py-4">
                      <p className="text-hp-paper/70">{a.contactFirstName} {a.contactLastName}</p>
                      <p className="text-hp-paper/45 text-xs">{a.email}</p>
                    </td>
                    <td className="px-4 py-4">
                    ...
                    <td className="px-4 py-4 text-hp-paper/45">{formatDate(a.createdAt)}</td>
                    <td className="px-4 py-4">
