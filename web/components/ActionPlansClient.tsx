...
const INITIAL_PLANS: ActionPlan[] = [
  {
    id: "ap1",
    title: "Front Knee Stability",
    priority: "High",
    status: "In Progress",
    dueDate: "2026-07-15",
    drills: [
      "Single-leg balance hold — 3 × 30s each side",
      "Lateral band walks — 3 × 15 reps",
      "Step-up with knee drive — 3 × 12 each leg",
    ],
    notes: "Focus on locking front knee at delivery stride. Review side-camera footage each session.",
  },
  {
    id: "ap2",
    ...
    dueDate: "2026-07-30",
    drills: [
      "Wall release drill — 20 reps daily",
      "Slow-motion delivery with coach feedback",
      "Video review against target release zone",
    ],
    notes: "Aim to keep release above 2.4m height. Use front-camera footage to track.",
  },
  {
    id: "ap3",
    title: "Yorker Line & Length",
    priority: "Low",
    status: "Pending",
    dueDate: "2026-08-20",
    drills: [
      "Target mat bowling — 30 deliveries per session",
      "Pressure situations drill with batsman",
    ],
    notes: "Begin this phase once release point goal is met.",
  },
];
...
export function ActionPlansClient({ player }: { player: Player }) {
  ...
  function saveNew() {
    ...
    setPlans((prev) => [...prev, { ...draft, drills: draft.drills.filter((d) => d.trim()) }]);
    setDraft(null);
    setAdding(false);
    setSaved(draft.id);
    setTimeout(() => setSaved(null), 2000);
