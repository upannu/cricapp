      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-3 mb-3">
            ...
            <span className="font-display font-black text-2xl tracking-wide text-hp-paper uppercase">CRIC HQ</span>
          </div>
          <p className="font-mono text-hp-paper/52 text-xs uppercase tracking-[0.2em]">Player Registration</p>
        </div>

        {!unlocked ? (
          <form onSubmit={handleUnlock} className="border border-white/12 bg-hp-surface p-8 space-y-4">
            <h2 className="font-display font-black uppercase text-lg text-hp-paper text-center mb-2 tracking-wide">Enter your registration code</h2>
            <p className="text-hp-paper/60 text-sm text-center mb-4">Ask your coach for the code if you don&apos;t have one.</p>
            <input
              type="text"
              value={code}
              onChange={(e) => { setCode(e.target.value); setCodeError(""); }}
              className={`w-full bg-hp-ink px-4 py-3 text-hp-paper placeholder-hp-paper/30 border focus:outline-none transition-colors text-sm text-center tracking-wide ${
                codeError ? "border-red-500" : "border-white/12 focus:border-hp-cg"
              }`}
          ...
          <div className="border border-white/12 bg-hp-surface p-8 text-center">
            <div className="w-16 h-16 rounded-full bg-hp-cg/10 border border-hp-cg/30 flex items-center justify-center mx-auto mb-5">
              <svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="#E8362A" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <path d="M20 6L9 17l-5-5" />
              </svg>
            </div>
            <h2 className="font-display font-black uppercase text-xl text-hp-paper mb-2 tracking-wide">Registered!</h2>
            ...
            <button
              ...
              className="w-full border border-white/15 text-hp-paper/80 font-bold py-3.5 hover:border-white/30 transition-colors text-sm uppercase tracking-wider cursor-pointer"
            >
              Register Another Player
            </button>
          </div>
        ) : selectedPlayerId === null ? (
          <div className="border border-white/12 bg-hp-surface p-8 space-y-4">
            <h2 className="font-display font-black uppercase text-lg text-hp-paper text-center mb-2 tracking-wide">Find your child</h2>
            <p className="text-hp-paper/60 text-sm text-center mb-2">
              Your coach already added these names — pick yours below to finish registering.
            </p>
            {pending === null ? (
              <p className="text-hp-paper/45 text-sm text-center py-4">Loading…</p>
            ) : (
              <div className="max-h-72 overflow-y-auto space-y-2 pr-1">
                {pending.map((p) => (
                  <button
                    ...
                    className="w-full text-left px-4 py-3 bg-hp-ink border border-white/12 hover:border-hp-cg text-hp-paper text-sm transition-colors cursor-pointer"
                  >
                    {p.name}
                  </button>
                ))}
            ...
            <button
              ...
              className="w-full text-center text-hp-paper/50 hover:text-hp-paper text-sm py-2 transition-colors cursor-pointer underline"
            >
              My child isn&apos;t listed — register them
            </button>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="border border-white/12 bg-hp-surface p-8 space-y-4">
            {selectedPlayerId && pending && pending.length > 0 && (
              <button
                type="button"
                onClick={() => setSelectedPlayerId(null)}
                className="text-xs text-hp-paper/50 hover:text-hp-paper transition-colors cursor-pointer -mt-1 -mb-2"
              >
                ← Back to the list
              </button>
            )}
            <h2 className="font-display font-black uppercase text-lg text-hp-paper text-center mb-2 tracking-wide">
              {selectedPlayerId ? "Finish Registration" : "Player Details"}
            </h2>
            {selectedPlayerId && (
              <p className="text-hp-paper/60 text-sm text-center -mt-2 mb-2">
                Just fill in the rest for <span className="text-hp-paper font-semibold">{form.name}</span>.
              </p>
            )}
            ...
            <div>
              <label className="block text-xs font-mono font-semibold text-hp-paper/52 uppercase tracking-widest mb-1.5">Parent/Player Email *</label>
              <input
                type="email"
                value={form.email}
                ...
                required
              />
              <p className="text-hp-paper/45 text-xs mt-1.5">Lets you sign in later to see progress — use this same email at /signup.</p>
            </div>

            <div>
              <label className="block text-xs font-mono font-semibold text-hp-paper/52 uppercase tracking-widest mb-1.5">Phone *</label>
              <input
                type="tel"
                value={form.phone}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select
                  ...
                  className="w-full bg-hp-ink px-4 py-3 text-hp-paper border border-white/12 focus:border-hp-cg focus:outline-none transition-colors text-sm"
                  required
                >
              ...
              <div>
                <label className="block text-xs font-mono font-semibold text-hp-paper/52 uppercase tracking-widest mb-1.5">Bowling Style *</label>
                <select
            ...
            <div>
              <label className="block text-xs font-mono font-semibold text-hp-paper/52 uppercase tracking-widest mb-1.5">Club (optional)</label>
              <input
                type="text"
                value={form.club}
        ...
        {registered && registered.length > 0 && (
          <div className="mt-8">
            <p className="text-xs font-mono font-semibold text-hp-paper/45 uppercase tracking-widest mb-3 text-center">
              {registered.length} player{registered.length === 1 ? "" : "s"} registered so far
            </p>
            <div className="border border-white/12 bg-hp-surface p-4 max-h-64 overflow-y-auto">
              <ul className="space-y-2">
                {registered.map((p, i) => (
                  <li key={i} className="flex items-center justify-between text-sm px-2 py-1.5">
                    <span className="text-hp-paper/85">{p.name}</span>
                    <span className="text-hp-paper/45 text-xs">{p.ageGroup}</span>
                  </li>
                ))}
              </ul>
            </div>
